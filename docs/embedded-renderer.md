...
#### Method A: Interactive Pairing Flow (Zero Hardcoding / Recommended for End Users)
1. **First Boot (Factory Unpaired State):**
   - The MCU checks NVS/Flash. If `device_id` and `device_token` are not found, it enters **Provisioning Mode**.
   - The MCU connects to the ScreenTinker WebSocket endpoint (`ws://<server>:3001/socket.io/?EIO=4&transport=websocket`).
   - The MCU generates a random 6-character/digit code (e.g. `947453`) and emits `device:register` with `{ pairing_code: "947453", device_info: { screen_width: 800, screen_height: 480 } }`.
   - The MCU renders the pairing code on its E-Paper display.
2. **Dashboard Claim:**
   - The user opens the ScreenTinker Web UI, goes to **Displays → Add Display**, and enters the code shown on the screen.
3. **Credential Stamping:**
   - The server acknowledges the claim by sending the `device:registered` event over the WebSocket: `{ device_id: "<UUID>", device_token: "<SECRET_TOKEN>", status: "online" }`.
   - The MCU writes `device_id` and `device_token` permanently to Flash/NVS.
   - The MCU disconnects the WebSocket permanently.
4. **Regular Operation:**
   - The MCU enters the low-power sleep/wake loop, using standard HTTP `GET /api/embedded/render` with its stored `device_token`.
...
3. The MCU starts directly in **Regular Operation** without ever needing the WebSocket pairing step.
